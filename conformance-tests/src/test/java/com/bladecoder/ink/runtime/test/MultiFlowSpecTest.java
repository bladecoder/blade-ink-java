...
public class MultiFlowSpecTest {
    ...
    public void basics() throws Exception {
        ...
        String json = compiler.compile(TestUtils.readFileAsString("inkfiles/runtime/multiflow-basics.ink"));
    ...
    public void testMultiFlowSaveLoadThreads() throws Exception {
        ...
        String json = compiler.compile(TestUtils.readFileAsString("inkfiles/runtime/multiflow-saveloadthreads.ink"));
        Story story = new Story(json);
        ...
        Assert.assertEquals(
                "Thread 1 blue choice", story.getCurrentChoices().get(0).getText());

        // Test existing state remains after switch (red)
        story.switchFlow("Red Flow");
        Assert.assertEquals("Hello I'm red\n", story.getCurrentText());
        Assert.assertEquals(
                "Thread 1 red choice", story.getCurrentChoices().get(0).getText());
