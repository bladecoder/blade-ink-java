...
public class VariableTextSpecTest {
	...
	public void sequence() throws Exception {
		...
		TestUtils.nextAll(story, text);
		Assert.assertEquals(1, text.size());
		Assert.assertEquals("The radio hissed into life. There was the white noise racket of an explosion.", text.get(0));
		story.chooseChoiceIndex(0);
	...
	public void listInChoice() throws Exception {
		...
		TestUtils.nextAll(story, text);
		Assert.assertEquals("\"Hello, Master!\"", story.getCurrentChoices().get(0).getText());
		story.chooseChoiceIndex(0);
		
		text.clear();
		TestUtils.nextAll(story, text);
		Assert.assertEquals("\"Hello, Monsieur!\"", story.getCurrentChoices().get(0).getText());
		story.chooseChoiceIndex(0);
